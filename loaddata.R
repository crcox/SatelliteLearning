load_session_data <- function(round,STUDY_INFO=STUDY_INFO) {
  require(stringr)
  datadir = file.path(STUDY_INFO$datadir,sprintf('round%02d',round))
  filelist = list.files(datadir)
  MetaData <- list()
  SessionData <- data.frame(subject=numeric(),session=numeric(),stage=factor(NULL,levels=1:3,labels=c('study','train','test')),start=as.Date(character()),end=as.Date(character()),accuracy=numeric())
  for (f in filelist) {
    print(f)
    ...
    if (filetype == "session") {
      ...
      X <- data.frame(
        ...
        session=1,
        stage=factor(rep(1:2,c(1,nrounds)),levels=1:3,labels=c('study','train','test')),
      ...
      for (i in 2:nsessions) {
        ...
        X <- data.frame(
          subject=subjectnumber,
          session=i,
          stage=factor(3,levels=1:3,labels=c('study','train','test')),
